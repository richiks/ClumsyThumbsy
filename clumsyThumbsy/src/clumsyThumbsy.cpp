/** This program uses recursive backtracking to autocorrect words
...
// function prototypes
Set<string> autocorrect(string word, Map<char, string> &nearLetters,
                        Lexicon &dictionary, int maxTypos);

Set<string> autocorrect(string remaining, Map<char, string> &nearLetters,
                        Lexicon &dictionary, int allowableTypos,
                        string builtUp);
...
int main() {
    ...
    while (true) {
        ...
        maxLettersIncorrect =
                getInteger("How many characters can be incorrect? ",
                                         "Please enter an integer");

        Set<string> correctedWords;
        correctedWords = autocorrect(wordToCorrect, nearLettersMap, dictionary,
                                     maxLettersIncorrect);
...
Set<string> autocorrect(string remaining, Map<char, string> &nearLetters,
    ...
    for (int i = 0; i < (int)nearLetters[curr].length(); i++) {
        result += autocorrect(rest, nearLetters, dictionary, allowableTypos - 1,
                              builtUp + nearLetters[curr][i]);
    }

    //can also choose not to change character
    result += autocorrect(rest, nearLetters, dictionary, allowableTypos,
                          builtUp + curr);
    return result;
}
